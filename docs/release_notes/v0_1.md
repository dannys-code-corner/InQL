...
- **Aggregate modifiers:** RFC 017 adds `AggregateMeasure.distinct()`, aggregate-local `filter(...)`, ordered-input representation with explicit rejection for current core aggregates, and compatibility helpers `count_distinct(...)` and `count_if(...)`. Distinct and filtered aggregate forms lower through Substrait and execute through the DataFusion-backed Session path.
