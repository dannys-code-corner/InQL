...
- **Approximate functions:** RFC 023 adds the first approximate aggregate slice with `approx_count_distinct(...)`. The helper is opt-in, marked approximate in registry metadata, lowers through InQL's registered Substrait `approx_count_distinct` aggregate extension name, and executes through the DataFusion-backed Session path via an adapter-local mapping to DataFusion's `approx_distinct` implementation.
