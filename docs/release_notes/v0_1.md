...
- **Aggregates:** builder-based `col`, `sum`, `count`, `count_expr`, `count_distinct`, `count_if`, `avg`, `min`, and `max` helpers now lower grouped and global aggregates through Prism, Substrait, and Session execution. `count()` counts rows, `count_expr(expr)` counts non-null expression values while preserving the future `count(expr)` semantics, and the first aggregate modifier slice supports `DISTINCT` plus aggregate-local `FILTER` where valid.
