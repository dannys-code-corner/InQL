...
- **Format functions:** RFC 022 adds scalar payload helpers for deterministic hashes (`md5`, `sha1`, `sha224`, `sha256`, `sha384`, `sha512`, `sha2`, `crc32`, and `xxhash64`), URL parsing/encoding/decoding, JSON validation/path/schema helpers, and CSV row/schema helpers. Format helpers lower through registry-owned Substrait metadata and execute through adapter-owned DataFusion UDF registration; dynamic variant predicates remain reserved until InQL defines the variant value model they inspect.
- **Approximate functions:** RFC 023 adds portable approximate aggregates with `approx_count_distinct(...)` and `approx_percentile(...)`. Both helpers are opt-in, marked approximate in registry metadata, lower through InQL's registered Substrait aggregate extension names, and execute through the DataFusion-backed Session path via adapter-local mappings to DataFusion aggregate implementations. Sketch-state helper names are delegated to RFC 025 rather than exposed as untyped binary/string payload functions.
